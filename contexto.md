...
| **Personalização** | Upload das logos do Header e da seção Sobre (bucket `logos`), com toggle "exibir em branco" (filtro `brightness-0 invert`) + edição de headline, highlight e subtítulo do Hero |
...
- **Storage Buckets:** `galeria` (imagens do showroom/projetos) e `logos` (logos do Header/Sobre — público)
